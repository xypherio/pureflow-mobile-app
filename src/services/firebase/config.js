...
// Note: Firebase config is nested under "extra.firebase" in app.json
...
// Debug log the resolved config (only in development)
if (__DEV__) {
...
if (missingConfigs.length > 0) {
  ...
  console.error('❌ Available config from app.json/extra:', Constants.expoConfig?.extra);
  console.error('❌ Available Firebase config:', Constants.expoConfig?.extra?.firebase);
  console.error('❌ Missing configurations:', missingConfigs);
  console.error('💡 Make sure Firebase config is nested under "extra.firebase" in app.json');
